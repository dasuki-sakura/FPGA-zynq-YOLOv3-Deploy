...
def _get_feats(feats, anchors, num_classes, input_shape):
    ...
    conf_threshold_logit = -5.0 
    mask = predictions[..., 4] > conf_threshold_logit
    ...
    box_xy = (1/(1+np.exp(-masked_predictions[:, :2])) + masked_grid) / np.array(grid_size[::-1], dtype=np.float32)
    
    box_wh = np.exp(masked_predictions[:, 2:4]) * masked_anchors / np.array(input_shape[::-1], dtype=np.float32)
    
    box_confidence = 1/(1+np.exp(-masked_predictions[:, 4:5]))
    
    box_class_probs = 1/(1+np.exp(-masked_predictions[:, 5:]))
...
def eval(yolo_outputs, image_shape, class_names, anchors):
    ...
    indices = cv2.dnn.NMSBoxes(boxes_for_cv, scores_for_cv, SCORE_THRESH, NMS_THRESH)
    ...
    if len(indices) > 0:
        ...
        boxes_ = valid_boxes[indices]
        scores_ = valid_scores[indices]
        classes_ = valid_classes[indices]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        filename_base = 'img{:0>5d}'.format(cnt)
        jpg_name = filename_base + '.jpg'
        npy_name = filename_base + '.npy'
        
        image_name = TEST_SET[list_idx] + '/' + jpg_name
        
        image_path = INPUT_IMAGE_PATH + image_name
        image = cv2.imread(image_path)
        
        data_path = PREPROCESSED_PATH + image_name.replace('.jpg', '.npy')
        ...
        post_processing_time = time.process_time() - post_processing_time_start
        #run_time = time.process_time() - total_time_start # <------------- Stop total Running Time Recording (1)
        run_time = preprocess_time + conv_time + post_processing_time
