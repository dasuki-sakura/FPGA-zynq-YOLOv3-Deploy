...
if [ -z "$SYSROOT" ]; then
    ...
    [ -f /usr/lib/dpu.xclbin ] || cp /media/sd-mmcblk1p1/dpu.xclbin /usr/lib/dpu.xclbin

    cp -d pkgs/usr/bin/*  /usr/bin/ || install_fail
    cp -d pkgs/usr/lib/*  /usr/lib/ || install_fail
    cp -d pkgs/usr/lib/colorsys.py /usr/lib/python3.7/ || install_fail
    ...
    cp -d pkgs/usr/lib/colorsys.py ${SYSROOT}/usr/lib/python3.7/ || install_fail
